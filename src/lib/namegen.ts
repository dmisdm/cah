...
let JAPANESE_NAMES_CONSTRAINED =
...
// Greek Names
let GREEK_NAMES = "<s<v|V>(tia)|s<v|V>(os)|B<v|V>c(ios)|B<v|V><c|C>v(ios|os)>";

// Hawaiian Names (1)
let HAWAIIAN_NAMES_1 =
...
let OLD_LATIN_PLACE_NAMES = "sv(nia|lia|cia|sia)";

// Dragons (Pern)
let DRAGONS_PERN = "<<s|ss>|<VC|vC|B|BVs|Vs>><v|V|v|<v(l|n|r)|vc>>(th)";

// Dragon Riders
let DRAGON_RIDERS = "c'<s|cvc>";

// Pokemon
let POKEMON = "<i|s>v(mon|chu|zard|rtle)";

// Fantasy (Vowels, R, etc.)
let FANTASY_VOWELS_R =
...
class Generator {
  ...
  constructor(
    //@ts-ignore
    pattern: string | Generator[] = null,
    collapse_triples: boolean = true
  ) {
...
const pokemonNameGenerator = new Generator([
  new Capitalizer(new Generator(POKEMON)),
]);
const fantasyNameGenerator = new Generator([
  new Capitalizer(new Generator(OLD_LATIN_PLACE_NAMES)),
]);
export const generateRandomPokemonName = () => pokemonNameGenerator.toString();
export const generateRandomFantasyName = () => fantasyNameGenerator.toString();
